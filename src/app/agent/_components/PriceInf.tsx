...
export const PriceInf = (props: PriceInf) => {
  ...
  const handleAgentImgInput = (e: React.ChangeEvent<HTMLInputElement>) => {
    const input = e.target;
    if (!input.files || input.files.length === 0 || agentImg.length >= 3)
      return;
    const file = input.files[0];
    setUploading(true);
    try {
      const imgUrl = URL.createObjectURL(file);
      setAgentImg((prev) => [...prev, imgUrl]);
      input.value = "";
    } catch (err) {
      console.log(err);
    } finally {
        ...
        <div className="p-6 space-y-4">
          <div>
            ...
            <input
              ...
              className="w-full px-4 py-2.5 border hover:border-blue-500 border-gray-300 rounded-lg focus:ring-2 focus:ring-green-500 focus:border-transparent outline-none transition-all"
            />
          </div>
          <div>
            <label className="block text-sm font-semibold text-gray-700 mb-2">
            ...
            <textarea
              ...
              rows={4}
              className="w-full hover:border-blue-500 px-4 py-2.5 border border-gray-300 rounded-lg focus:ring-2 focus:ring-green-500 focus:border-transparent outline-none transition-all resize-none"
            />
          </div>
          <div>
            <label className="block text-sm font-semibold text-gray-700 mb-2">
            ...
            <div className="flex gap-3">
              {agentImg.map((img, index) => (
                ...
                >
                  ...
                  <button
                    className="absolute top-2 right-2 bg-red-500 hover:bg-red-600 w-6 h-6 rounded-lg flex justify-center items-center text-white shadow-lg opacity-0
